...
홈
-> 기기추가 (플러스버튼, 스마트 선반만 추가가능)
-> 기기 온오프 소개 (스마트선반만 가능)
-> 알림 버튼 소개 (더미데이터)
->스마트 선반 클릭 (식품이 등록되어있다는 가정이기 때문에 점등 화면 사용, 발표자료의 시나리오와 다른부분)
-> 다른 선반 비어있는거 확인

-> 메인화면으로 이동 (KING)
-> 돋보기 기능 확인 (가지 검색 확인하고 가지 눌러서 다시 메인으로)
-> 인포기능 확인
-> 하이라이트 기능 확인
-> 정렬순서 확인하고 다시 메인으로 (뒤로가기가 아니라 다시 가나다순 누르면 됨)

-> 메인화면에서 가지 왼쪽으로 슬라이드
-> 세부정보확인
-> 해당 페이지에서 편집클릭
-> 유통기한 토글버튼, 장기미사용 버튼, 취소버튼 소개 (이 화면에서 편집 저장은 기능 없음 유의, 전화면으로 돌아감)
-> 식품삭제 클릭 (가지 삭제)
-> 삭제 확인 팝업에서 취소 가능, but 일단 삭제
-> 메인화면으로 돌아감 (가지 사라진거 확인 가능)

-> 감을 왼쪽으로 슬라이드 (인포버튼 기능 없음 주의)
-> 감 삭제 
